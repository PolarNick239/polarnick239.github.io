...
```Опр``` Векторным произведением $$\vec{u} \times \vec{v}$$ называется произведение длин векторов и знакового синуса ориентированного угла (см. правило буравчика) $$a$$ от $$\vec{u}$$ к $$\vec{v}$$:
...
$$\operatorname{LeftTurn}(A, B, C) =\left\{
                                         \begin{array}{rl}
                                         1 &\mbox{, if}\ \vec{AB}\times\vec{AC} > 0\\
                                         -1 &\mbox{, if}\ \vec{AB}\times\vec{AC} < 0\\
                                         0 &\mbox{, if}\ \vec{AB}\times\vec{AC} ~= 0
                                         \end{array}$$ - предикат левого поворота.
...
Иначе говоря если ввести $$\vec{u}=\vec{AB}$$ и $$\vec{v}=\vec{AC}$$, то предикат левого поворта:
 ...
 - **Положителен** если мы поворачиваем **против часовой стрелки** при движении от $$\vec{u}=\vec{AB}$$ к $$\vec{v}=\vec{AC}$$
...
Что такое $$\operatorname{LeftTurn}(A, B, C)$$? Замети что предикат левого поворта:
...
А что такое $$\operatorname{LeftTurn}(A, B, D)$$? То же самое, но на этот раз про точку $$D$$ относительно отрезка $$AB$$.

```Утв 1``` Отрезки $$AB$$ и $$CD$$ точно не пересекаются, если и $$С$$ и $$D$$ лежат по одну и ту же сторону от отрезка $$AB$$.
...
```Утв 3``` $$\operatorname{isIntersected}(AB, CD) = (\operatorname{LeftTurn}(A, B, C) != \operatorname{LeftTurn}(A, B, D)) && (\operatorname{LeftTurn}(C, D, A) != \operatorname{LeftTurn}(C, D, B))$$
