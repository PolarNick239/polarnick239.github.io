...
Одной из удобных оценок времени работы алгоритма является способ оценить сколько примерно примитивных операций $$f(n)$$ выполнит алгоритм
в зависимости от объема входных данных $$n$$. При этом не так важна точная оценка числа операций - достаточно чтобы функция $$f(n)$$ с ростом $$n$$ 
росла сопоставимо с тем как растет точное число операций. Чуть более строго - $$f(n)$$ должна расти не быстрее чем точное число операций.
...
такая константа $$c > 0$$, что $$f(n) \lt c \dot g(n)$$ для всех натуральных $$n$$.
...
4) Любой полином растет быстрее логарифма, например $$n$$ и даже $$\sqrt(n)$$ растет быстрее чем $$(log n)^3$$. **Например** $$O(n+log n) = O(n)$$

Примеры асимптотики работы Java-классов
======
...
values.add(0, scanner.nextInt()); // эта операция вставит считанное число перед бывшим первым элементом, иначе говоря - сдвинет весь массив на единицу вправо,
                                  ...
                                  // процедура сдвига занимает столько времени сколько элементов надо скопировать, т.е. O(values.size())=O(n+1)=O(n)
```
...
int x = values.get(n / 2); // эта операция интересная, чтобы дойти до середины - надо начиная с первого элемента перейти по n/2-1 ссылке
                           // от элемента к элементу, т.е. занимает O(n/2-1)=O(n)
...
```Упр. 3``` Какая асимптотика у алгоритма умножения столбиком? В предположении что число цифр в обоих числах $$N$$, и что умножить/сложить цифру на цифру и т.п. - $$O(1)$$.
...
for (int i = 0; i < N; ++i) {
    ...
    result += scanner.nextInt(); // O(A.length + B.length)
}
```
...
for (int i = 0; i < N; ++i) {
    values.add(0, scanner.nextInt()); // выполняет вставку значения перед самым первым элементом
                                      ...
                                      // т.о. данный add выполняется за O(values.size())
}
```
...
(http://www.math.nsc.ru/LBRT/k5/OR-MMF/dasgupta_2014.pdf)[Дасгупта С., Пападимитриу Х., Вазирани У., «Алгоритмы»]
