...
title:  "Геометрия: абсолютная точность в предикате левого поворота, принадлежность точки полигону"
...
Т.е. если число $$X = -234500$$, то мантисса хранит первые несколько значащих цифр (столько сколько влизает в биты выделенные под мантиссу), а показатель (экспонента) кодирует информацию о том где находится запятая, т.е. иначе говоря о том какой порядок у данного числа.
