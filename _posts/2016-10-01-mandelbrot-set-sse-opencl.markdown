...
<!-- TODO: add image of green Mandelbrot -->
...
We want to highlight such points on $$2D$$ plane that corresponding complex numbers belongs to the Mandelbrot set:
...
Let's implement and tune performance of function calculating required number of iterations per pixel in ```mandelbrotContains(x, y)``` function. If that number is equal to ```MAX_ITERATIONS``` - 
 that pixel belongs to the Mandelbrot set.
 
Implementation on **C++** looks like this part of {% include icon-github.html label="code:" link="https://github.com/PolarNick239/FPGABenchmarks/blob/c36b213bed3fbf6c714f3a819e820d3d393c9711/benchmarks/mandelbrot/src/mandelbrot_cpu.cpp#L13-L45" %}
...
Notes: ```from``` and ```to``` are the coordinates of image corners on complex plane, ```INFINITY``` - is divergence threshold, for the Mandelbrot set it is equal to ```2.0f```. 
...
But this CPU has 4 cores, and 8 threads because of hyper-threading, so let's crunch numbers in 8 threads by using **OpenMP**. Now this is really the same as on {% include icon-github.html label="github:" link="https://github.com/PolarNick239/FPGABenchmarks/blob/c36b213bed3fbf6c714f3a819e820d3d393c9711/benchmarks/mandelbrot/src/mandelbrot_cpu.cpp#L13-L45" %}
...
void MandelbrotProcessorCPU::process(Vector2f from, Vector2f to,
    ...
    // The only difference to previous implementation is the following line:
...
In this case **OpenMP** takes care of distributing workload between all available cores, because it was asked to do so on outer for-loop - it will execute on each thread code-block of outer loop for some ```py```.
...
The same benchmark shows **1274 ms**. The speedup is ```5545 ms / 1274 ms = 4.3``` times. It is not close to **x8** because there are in fact only **4** real cores.
 ...
 **SIMD** (Single Instruction, Multiple Data) makes possible to do some operation simultaneously on multiple operands.
 For example such instructions can calculate sum of $$a_1,a_2,a_3,a_3$$ and $$b_1,b_2,b_3,b_4$$, so the result is $$c_1,c_2,c_3,c_4$$, where $$c_i=a_i+b_i$$.
...
All modern compiles supports **intrinsics** for such extensions.
 ...
 stores result in another register. More detailed description about all intrinsics can be found on very handy [Intel intrinsics guide](https://software.intel.com/sites/landingpage/IntrinsicsGuide/#text=_mm_add_ps&expand=109).
...
Because intrinsics works with **4 operands** at time - we should make our computations with them in CPU register, than unload data from result register to memory.

**C++** compiler will take care about mapping of our variables into registers, but we should help him by explicitly working with 4 values at time - by using special type: ```__m128``` - **128-bits** register.
...
void MandelbrotProcessorCPU_SSE::process(Vector2f from, Vector2f to,
    ...
    for (size_t py = 0; py < iterations.height; py++) {
        ...
        for (size_t px = 0; px < iterations.width / 4 * 4; px += 4) {
            ...
            __m128 xs0 = _mm_add_ps(_mm_set1_ps(from.x()), _mm_add_ps(_mm_set1_ps(x_step * pxf), pxs_deltas128));   // from.x()+px*x_step, where px takes 4 values from current loop index
...
Yes! Such a mess! Coding this is very slow (of course if you do it only on at times for optimization). [Intel intrinsics guide](https://software.intel.com/sites/landingpage/IntrinsicsGuide/#text=_mm_add_ps&expand=109)
 helps a lot - it has very good filters by extensions (for example you may be not wanting suddenly use **AVX** because not all of your consumers have CPU, that supports it), provides very strict descriptions and so on.
 
The final for-loop is a fallback to naive CPU version, because we may have number of data, that is not a multiple of 4, and this way is the simplest one to deal with that values.
...
But we can go futher! After **SSE** new instructions extension was **AVX** (described in 2008, supported since 2011 in **Sandy Bridge**).
 The main difference is usage of wider registers - **256-bits** wide, so it can be twice faster! Note, that when code is adopted to **SSE** usage - adoptation to **AVX**
 is quite straightforward - just replace ```__m128``` with ```__m256```, and update code to minor functions changes (sometimes order of arguments changes...).
...
This part will be very easy and give a VERY huge speed up on GPU!
